...
if __name__ == '__main__':
    ...
    for fold, (train_idx_fold, val_idx_fold) in enumerate(skf.split(train_val_idx, y_all[train_val_idx])):
        ...
        # Get predictions from all 4 CNNs on validation set
        ...
        # Simple Average of 4 CNN Predictions
        avg_preds_val = (resnet18_val + convnexttiny_val + efficientnetv2s_val + densenet121_val) / 4.0
        
        predicted_labels = (avg_preds_val >= 0.5).astype(int)
        
        # Calculate metrics
        auc = roc_auc_score(y_val_fold, avg_preds_val)
    ...
    # Get predictions from all 4 CNNs on test set
    ...
    # Simple Average of 4 CNN Predictions
    avg_preds_test = (resnet18_test + convnexttiny_test + efficientnetv2s_test + densenet121_test) / 4.0
    
    # Use fixed 0.5 threshold
    test_preds = (avg_preds_test >= 0.5).astype(int)
    
    # Calculate test metrics
    test_auc = roc_auc_score(y_test, avg_preds_test)
