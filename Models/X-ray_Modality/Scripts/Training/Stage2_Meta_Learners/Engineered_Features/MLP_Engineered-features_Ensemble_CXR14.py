...
# Function to engineer features from CNN predictions
def engineer_features(cnn_probs):

    n_samples = cnn_probs.shape[0]
    features = np.zeros((n_samples, 11))
    
    # Original 4 CNN probabilities
    features[:, 0:4] = cnn_probs
    
    # Derived features
    features[:, 4] = np.mean(cnn_probs, axis=1)  # mean - consensus strength
    features[:, 5] = np.var(cnn_probs, axis=1)   # var - disagreement/uncertainty
    features[:, 6] = np.min(cnn_probs, axis=1)   # min - pessimistic bound
    features[:, 7] = np.max(cnn_probs, axis=1)   # max - optimistic bound
    features[:, 8] = features[:, 7] - features[:, 6]  # range - spread
    features[:, 9] = np.sum(cnn_probs >= 0.5, axis=1)  # vote count - majority rule
    
    # mean entropy - confidence measure
    epsilon = 1e-10  
...
if __name__ == '__main__':
    ...
    output_dir = "./MLP_Engineered-features_Ensemble"
    ...
    for fold, (train_idx_fold, val_idx_fold) in enumerate(skf.split(train_val_idx, y_all[train_val_idx])):
        ...
        # Stack CNN probabilities and engineered features
        cnn_probs_train = np.column_stack([resnet18_train, convnexttiny_train, efficientnetv2s_train, densenet121_train])
        X_train = engineer_features(cnn_probs_train)
        ...
        # Stack CNN probabilities and engineered features
        cnn_probs_val = np.column_stack([resnet18_val, convnexttiny_val, efficientnetv2s_val, densenet121_val])
        X_val = engineer_features(cnn_probs_val)
        ...
        # Find optimal ROC-AUC hyperparameters for each individual fold (based on training set) 
        print("Running GridSearchCV for hyperparameter tuning...")
        mlp_base = MLPClassifier(random_state=42, max_iter=500, early_stopping=True, validation_fraction=0.1)
        ...
        # Optimize threshold on training set for F1 score
        print("Optimizing F1 threshold on training set...")
        train_preds_proba = mlp_model.predict_proba(X_train)[:, 1]
        optimal_threshold, train_f1 = optimize_threshold(y_train_fold, train_preds_proba)
        print(f"Optimal threshold: {optimal_threshold:.3f} (Train F1: {train_f1:.4f})")
        
        # Predict on validation fold using optimized threshold
        mlp_preds_proba = mlp_model.predict_proba(X_val)[:, 1]
        predicted_labels = (mlp_preds_proba >= optimal_threshold).astype(int)
        
        # Calculate metrics
        auc = roc_auc_score(y_val_fold, mlp_preds_proba)
    ...
    # Get CNN predictions on full train+val
    ...
    # Stack CNN probabilities and engineered features
    cnn_probs_trainval = np.column_stack([resnet18_trainval, convnexttiny_trainval, efficientnetv2s_trainval, densenet121_trainval])
    X_trainval = engineer_features(cnn_probs_trainval)
    ...
    # Find optimal ROC-AUC hyperparameters on all train+val data 
    mlp_base = MLPClassifier(random_state=42, max_iter=500, early_stopping=True, validation_fraction=0.1)
    ...
    # Get best model
    final_mlp_model = grid_search_final.best_estimator_
    print(f"Best hyperparameters: {grid_search_final.best_params_}")
    print(f"Best CV AUC score: {grid_search_final.best_score_:.4f}")
    
    # Optimize threshold on full train+val set for F1 score
    trainval_preds_proba = final_mlp_model.predict_proba(X_trainval)[:, 1]
    final_threshold, _ = optimize_threshold(y_trainval, trainval_preds_proba)
    ...
    # Stack CNN probabilities and engineered features
    cnn_probs_test = np.column_stack([resnet18_test, convnexttiny_test, efficientnetv2s_test, densenet121_test])
    X_test = engineer_features(cnn_probs_test)
    
    # Predict on test set using optimized threshold
    test_preds_proba = final_mlp_model.predict_proba(X_test)[:, 1]
