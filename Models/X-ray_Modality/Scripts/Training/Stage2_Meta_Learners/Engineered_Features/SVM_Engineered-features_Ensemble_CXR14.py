...
if __name__ == '__main__':
    ...
    output_dir = "./SVM_Engineered-features_Ensemble"
    ...
    for fold, (train_idx_fold, val_idx_fold) in enumerate(skf.split(train_val_idx, y_all[train_val_idx])):
        ...
        svm_base = SVC(random_state=42, probability=True, class_weight='balanced')
        ...
        train_preds_proba = svm_model.predict_proba(X_train)[:, 1]
        ...
        svm_preds_proba = svm_model.predict_proba(X_val)[:, 1]
        predicted_labels = (svm_preds_proba >= optimal_threshold).astype(int)
        
        # Calculate metrics
        auc = roc_auc_score(y_val_fold, svm_preds_proba)
    ...
    # Get best model
    final_svm_model = grid_search_final.best_estimator_
    ...
    trainval_preds_proba = final_svm_model.predict_proba(X_trainval)[:, 1]
    ...
    test_preds_proba = final_svm_model.predict_proba(X_test)[:, 1]
