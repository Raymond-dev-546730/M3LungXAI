...
if __name__ == '__main__':
    ...
    output_dir = "./Ensemble_Model_CXR14"
    ...
    for fold, (train_idx_fold, val_idx_fold) in enumerate(skf.split(train_val_idx, y_all[train_val_idx])):
        ...
        # Stack into feature matrix (4 base features only)
    ...
    model_save_path = os.path.join(output_dir, 'GB_Model.joblib')
    joblib.dump(final_gb_model, model_save_path)
    print(f"Saved Gradient Boosting model to: {model_save_path}")
    
    # Create configuration dictionary 
    config = {
        'gb_threshold': float(final_threshold),
        'feature_order': ['ResNet18', 'ConvNeXtTiny', 'EfficientNetV2S', 'DenseNet121']
    }
